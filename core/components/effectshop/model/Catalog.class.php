...
class Catalog
{
	...
	{
		$cache = Shop::fromCache('product_', $id, 'resource');
		...
		$qs = self::getQuerySelect($getFields, $resFields, $tvsFields);

		$q = $modx->newQuery('modResource');
		$q->select($qs['select']);
		foreach ($qs['joins'] as $join) {
		...
		$p = self::processProducts([$product]);

		Shop::toCache($p[0], 'product_', $id, 'resource');
		return $p[0];
	}
